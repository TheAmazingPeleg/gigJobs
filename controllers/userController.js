...
const createUser = catchAsync(async (req, res, next) => {
    ...
    if(!username || !validator.isAlphanumeric(username) || !validator.isLength(username, { min: 3, max: 25 }))
        next(new AppError('The username should contains only abc letters and numbers between 3 to 25 letters', 400));
    if(!email || !validator.isEmail(email))
        next(new AppError('The email should be in the email`s format', 400));
    if(!firstName || !validator.isAlpha(firstName) || !validator.isLength(firstName, { min: 2, max: 30 }))
        next(new AppError('The first name should contains only abc letters between 2 to 30 letters', 400));
    if(!lastName || !validator.isAlpha(lastName) || !validator.isLength(lastName, { min: 2, max: 30 }))
        next(new AppError('The last name should contains only abc letters between 2 to 30 letters', 400));
    if(!age || !validator.isNumeric(ageStr) || !(parseInt(ageStr) > 15 && parseInt(ageStr) < 100))
        next(new AppError('The age should contains only numbers between 16 to 99', 400));
    if(bio && !validator.isLength(bio, { max: 250 }))
        next(new AppError('The bio should be less then 250 letters', 400));
    if(!photo || !validator.isURL(photo))
        next(new AppError('The photo should be URL', 400));
    if(!password || !passwordConfirm || !validator.isLength(password, { min: 8, max: 250 }) || password != passwordConfirm)
        next(new AppError('The password should be between 8 to 250 and matchs', 400));
    if((await findByUsername(username)).length != 0)
        next(new AppError('The username is taken', 400));
    if((await findByEmail(email)).length != 0)
        next(new AppError('The email is taken', 400));
...
const login = catchAsync(async (req, res, next) => {
    ...
    if(!username || !password)
        next(new AppError('Please provide username/email and password', 401));
    if(user.length === 0)
        user = await findByUsername(username);
    if(user.length === 0)
        next(new AppError('No user exists!', 401));
    user = user[0];
    if(!user.currectPassword(password))
        next(new AppError('The password is wrong!', 401));
